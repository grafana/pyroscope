...
func (m *HeatmapCell) CloneVT() *HeatmapCell {
	if m == nil {
		return (*HeatmapCell)(nil)
	}
	r := new(HeatmapCell)
	r.TimeStart = m.TimeStart
	r.ValueStart = m.ValueStart
	r.ValueEnd = m.ValueEnd
	r.Count = m.Count
...
func (m *HeatmapCell) CloneMessageVT() proto.Message {
	return m.CloneVT()
}
...
func (this *HeatmapCell) EqualVT(that *HeatmapCell) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	...
	if this.ValueStart != that.ValueStart {
		return false
	}
	if this.ValueEnd != that.ValueEnd {
		return false
	}
...
func (this *HeatmapCell) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*HeatmapCell)
	if !ok {
		return false
	}
	return this.EqualVT(that)
...
func (m *HeatmapCell) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *HeatmapCell) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *HeatmapCell) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if m.ValueEnd != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.ValueEnd))
		i--
		dAtA[i] = 0x18
	}
	if m.ValueStart != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.ValueStart))
		i--
		dAtA[i] = 0x10
	}
	if m.TimeStart != 0 {
...
func (m *HeatmapCell) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	if m.ValueStart != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.ValueStart))
	}
	if m.ValueEnd != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.ValueEnd))
	}
	if m.Count != 0 {
...
func (m *HeatmapCell) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: HeatmapCell: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: HeatmapCell: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
		...
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field ValueStart", wireType)
			}
			m.ValueStart = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.ValueStart |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field ValueEnd", wireType)
			}
			m.ValueEnd = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.ValueEnd |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
