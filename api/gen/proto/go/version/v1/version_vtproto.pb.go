...
func (m *VersionResponse) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.QuerierAPI != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.QuerierAPI))
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
...
func (m *VersionResponse) SizeVT() (n int) {
	...
	_ = l
	if m.QuerierAPI != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.QuerierAPI))
	}
	n += len(m.unknownFields)
	return n
}
...
func (m *InstanceVersion) UnmarshalVTUnsafe(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
...
func (m *Versions) UnmarshalVTUnsafe(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
