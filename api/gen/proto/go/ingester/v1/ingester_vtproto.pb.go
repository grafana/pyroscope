...
func (m *ProfileSets) CloneVT() *ProfileSets {
	...
	if rhs := m.LabelsHashes; rhs != nil {
		tmpContainer := make([]uint64, len(rhs))
		copy(tmpContainer, rhs)
		r.LabelsHashes = tmpContainer
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (this *ProfileSets) EqualVT(that *ProfileSets) bool {
	...
	if len(this.LabelsHashes) != len(that.LabelsHashes) {
		return false
	}
	for i, vx := range this.LabelsHashes {
		vy := that.LabelsHashes[i]
		if vx != vy {
			return false
		}
	}
...
func (m *ProfileSets) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.LabelsHashes) > 0 {
		var pksize2 int
		for _, num := range m.LabelsHashes {
	...
	for iNdEx < l {
		...
		case 3:
			if wireType == 0 {
				...
				m.LabelsHashes = append(m.LabelsHashes, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				elementCount = count
				if elementCount != 0 && len(m.LabelsHashes) == 0 {
					m.LabelsHashes = make([]uint64, 0, elementCount)
				}
				for iNdEx < postIndex {
					var v uint64
					for shift := uint(0); ; shift += 7 {
				...
				return fmt.Errorf("proto: wrong wireType = %d for field LabelsHashes", wireType)
			}
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
