//
// Created by korniltsev on 12/8/23.
//

#ifndef PYROEBPF_PYPERF_COMMON_H
#define PYROEBPF_PYPERF_COMMON_H
...
static __always_inline py_event *pyperf_collect_impl(void *ctx, pid_t pid, bool collect_kern_stack) {
    ...
    if (get_thread_state(pid_data, &thread_state)) {
        submit_error_sample(state, PY_ERROR_THREAD_STATE);
        return NULL;
    }
    ...
    if (thread_state != 0) {
        if (get_top_frame(pid_data, state, thread_state)) {
            submit_error_sample(state, PY_ERROR_TOP_FRAME);
            return NULL;
        }

        return event; // and then tail call
    }
...
#endif //PYROEBPF_PYPERF_COMMON_H
