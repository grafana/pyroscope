...
var pyVersions = map[Version]*UserOffsets{`
	fmt.Println(header)
	for _, e := range es {
		fmt.Printf("// %d.%d.%d %s\n", e.PythonVersion.Major, e.PythonVersion.Minor, e.PythonVersion.Patch, e.SrcFile)
		fmt.Printf("{%d, %d, %d}:  {\n", e.PythonVersion.Major, e.PythonVersion.Minor, e.PythonVersion.Patch)
