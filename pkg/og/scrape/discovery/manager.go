...
func (m *Manager) startProvider(ctx context.Context, p *provider) {
	m.logger.
		...
		WithField("subs", fmt.Sprintf("%v", p.subs)).
...
func (m *Manager) updater(ctx context.Context, p *provider, updates chan []*targetgroup.Group) {
	...
	for {
		...
		case tgs, ok := <-updates:
			...
			if !ok {
				m.logger.WithField("provider", p.name).Debug("discoverer channel closed")
		...
		case <-ticker.C: // Some discoverers send updates too often, so we throttle these with the ticker.
			...
			case <-m.triggerSend:
				...
				default:
					...
					m.logger.Debugf("discovery receiver's channel was full so will retry the next cycle")
					select {
					case m.triggerSend <- struct{}{}:
					default:
					}
...
func (m *Manager) registerProviders(cfgs Configs, setName string) int {
	...
	add := func(cfg Config) {
		...
		d, err := cfg.NewDiscoverer(DiscovererOptions{
			Logger: m.logger.WithField("discovery", typ),
		})
		if err != nil {
			m.logger.WithError(err).
				WithField("type", typ).
				Errorf("cannot create service discovery")
			failed++
			return
		}
