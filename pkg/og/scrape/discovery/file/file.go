...
import (
	...
	"github.com/grafana/pyroscope/pkg/og/scrape/config"
	"github.com/grafana/pyroscope/pkg/og/scrape/discovery"
	"github.com/grafana/pyroscope/pkg/og/scrape/discovery/targetgroup"
	pmodel "github.com/grafana/pyroscope/pkg/og/scrape/model"
...
var (
	fileSDScanDuration = prometheus.NewSummary(
		prometheus.SummaryOpts{
			Name:       "pyroscope_sd_file_scan_duration_seconds",
			Help:       "The duration of the File-SD scan in seconds.",
			Objectives: map[float64]float64{0.5: 0.05, 0.9: 0.01, 0.99: 0.001},
		})
	fileSDReadErrorsCount = prometheus.NewCounter(
		prometheus.CounterOpts{
			Name: "pyroscope_sd_file_read_errors_total",
...
func NewTimestampCollector() *TimestampCollector {
	return &TimestampCollector{
		Description: prometheus.NewDesc(
			"pyroscope_sd_file_mtime_seconds",
...
func (d *Discovery) listFiles() []string {
	...
	for _, p := range d.paths {
		...
		if err != nil {
			d.logger.WithError(err).WithField("glob", p).Error("Error expanding glob")
			continue
		}
...
func (d *Discovery) watchFiles() {
	...
	for _, p := range d.paths {
		...
		if err := d.watcher.Add(p); err != nil {
			d.logger.WithError(err).WithField("path", p).Error("Error adding file watch")
		}
	}
}
...
func (d *Discovery) Run(ctx context.Context, ch chan<- []*targetgroup.Group) {
	...
	if err != nil {
		d.logger.WithError(err).Error("Error adding file watcher")
		return
	}
	d.watcher = watcher
	defer d.stop()
	...
	for {
		...
		case err := <-d.watcher.Errors:
			if err != nil {
				d.logger.WithError(err).Error("Error watching file")
			}
		}
	}
}
...
func (d *Discovery) stop() {
	d.logger.WithField("paths", fmt.Sprintf("%v", d.paths)).Debug("Stopping file discovery...")
	...
	if err := d.watcher.Close(); err != nil {
		d.logger.WithError(err).WithField("paths", fmt.Sprintf("%v", d.paths)).Error("Error closing file watcher")
	}
...
func (d *Discovery) refresh(ctx context.Context, ch chan<- []*targetgroup.Group) {
	...
	for _, p := range d.listFiles() {
		...
		if err != nil {
			...
			d.logger.WithField("path", p).WithError(err).Debug("Error reading file")
	...
	for f, n := range d.lastRefresh {
		m, ok := ref[f]
		if !ok || n > m {
			d.logger.Debug("msg", "file_sd refresh found file that should be removed", "file", f)
			d.deleteTimestamp(f)
			for i := m; i < n; i++ {
				select {
...
func (d *Discovery) readFile(filename string) ([]*targetgroup.Group, error) {
	...
	for i, tg := range targetGroups {
		...
		tg.Labels[fileSDFilepathLabel] = pmodel.LabelValue(filename)
	}
