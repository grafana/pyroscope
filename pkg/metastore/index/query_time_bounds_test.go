...
// Trimmed-message decoding is only sound if every view field matches its
// BlockMeta counterpart in name, number, and type.
func TestBlockMetaTimeBounds_FieldNumbers(t *testing.T) {
	source := (&metastorev1.BlockMeta{}).ProtoReflect().Descriptor().Fields()
	view := (&indexpb.BlockMetaTimeBounds{}).ProtoReflect().Descriptor().Fields()
	require.Positive(t, view.Len())
	for i := range view.Len() {
		f := view.Get(i)
		src := source.ByName(f.Name())
		require.NotNil(t, src, "field %s is not part of BlockMeta", f.Name())
		require.Equal(t, src.Number(), f.Number(), "field %s", f.Name())
		require.Equal(t, src.Kind(), f.Kind(), "field %s", f.Name())
	}
}
...
func TestBlockMetaTimeBounds(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			minTime, maxTime := blockMetaTimeBounds(b)
...
func TestBlockMetaTimeBounds_MalformedInput(t *testing.T) {
	...
	for i := 0; i <= len(b); i++ {
		...
		blockMetaTimeBounds(b[:i])
	}
	blockMetaTimeBounds([]byte{0xff, 0xff, 0xff, 0xff})
}
