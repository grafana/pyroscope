...
func IterateRaw(r io.Reader, buf []byte, cb func(k []byte, v int)) error {
	...
	for {
		...
		if descCount != 0 {
			// Add node suffix and save offset.
		...
		// Cut the current label.
		b.Truncate(b.Len() - int(copied))
		// Cut parent suffix, if it has no more descendants.
		i := len(offsets) - 1
		if i < 0 {
			continue
		}
		...
		for ; i > 0; i-- {
			...
			// No descending nodes left. Cut suffix and remove the offset.
