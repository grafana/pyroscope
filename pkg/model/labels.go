...
// instantiation.
type Labels []*typesv1alpha1.LabelPair
...
func (ls Labels) WithoutPrivateLabels() Labels {
	res := make([]*typesv1alpha1.LabelPair, 0, len(ls))
	for _, l := range ls {
...
func (ls Labels) Clone() Labels {
	...
	for i, l := range ls {
		result[i] = &typesv1alpha1.LabelPair{
			Name:  l.Name,
			Value: l.Value,
		}
	}
...
func LabelPairsString(lbs []*typesv1alpha1.LabelPair) string {
	var b bytes.Buffer
	b.WriteByte('{')
	for i, l := range lbs {
		if i > 0 {
...
func StringToLabelsPairs(s string) ([]*typesv1alpha1.LabelPair, error) {
	matchers, err := parser.ParseMetricSelector(s)
	if err != nil {
		return nil, err
	}
	result := make([]*typesv1alpha1.LabelPair, len(matchers))
...
func LabelsFromStrings(ss ...string) Labels {
	...
	for i := 0; i < len(ss); i += 2 {
		res = append(res, &typesv1alpha1.LabelPair{Name: ss[i], Value: ss[i+1]})
	}
...
func CloneLabelPairs(lbs []*typesv1alpha1.LabelPair) []*typesv1alpha1.LabelPair {
	result := make([]*typesv1alpha1.LabelPair, len(lbs))
	for i, l := range lbs {
		result[i] = &typesv1alpha1.LabelPair{
			Name:  l.Name,
			Value: l.Value,
...
func CompareLabelPairs(a []*typesv1alpha1.LabelPair, b []*typesv1alpha1.LabelPair) int {
	l := len(a)
	if len(b) < l {
		l = len(b)
	}
...
func NewLabelsBuilder(base Labels) *LabelsBuilder {
	b := &LabelsBuilder{
		del: make([]string, 0, 5),
		add: make([]*typesv1alpha1.LabelPair, 0, 5),
	}
	b.Reset(base)
	return b
}
...
func (b *LabelsBuilder) Set(n, v string) *LabelsBuilder {
	...
	b.add = append(b.add, &typesv1alpha1.LabelPair{Name: n, Value: v})
