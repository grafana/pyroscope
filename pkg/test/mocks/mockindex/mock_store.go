...
import (
	...
	store "github.com/grafana/pyroscope/v2/pkg/metastore/index/store"
...
// LoadShardVersion provides a mock function with given fields: tx, p, tenant, shard
func (_m *MockStore) LoadShardVersion(tx *bbolt.Tx, p store.Partition, tenant string, shard uint32) (uint64, error) {
	ret := _m.Called(tx, p, tenant, shard)

	if len(ret) == 0 {
		panic("no return value specified for LoadShardVersion")
	}

	var r0 uint64
	var r1 error
	if rf, ok := ret.Get(0).(func(*bbolt.Tx, store.Partition, string, uint32) (uint64, error)); ok {
		return rf(tx, p, tenant, shard)
	}
	if rf, ok := ret.Get(0).(func(*bbolt.Tx, store.Partition, string, uint32) uint64); ok {
		r0 = rf(tx, p, tenant, shard)
	} else {
		r0 = ret.Get(0).(uint64)
	}
...
// MockStore_LoadShardVersion_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'LoadShardVersion'
type MockStore_LoadShardVersion_Call struct {
	*mock.Call
}

// LoadShardVersion is a helper method to define mock.On call
//   - tx *bbolt.Tx
//   - p store.Partition
//   - tenant string
//   - shard uint32
func (_e *MockStore_Expecter) LoadShardVersion(tx interface{}, p interface{}, tenant interface{}, shard interface{}) *MockStore_LoadShardVersion_Call {
	return &MockStore_LoadShardVersion_Call{Call: _e.mock.On("LoadShardVersion", tx, p, tenant, shard)}
}

func (_c *MockStore_LoadShardVersion_Call) Run(run func(tx *bbolt.Tx, p store.Partition, tenant string, shard uint32)) *MockStore_LoadShardVersion_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(*bbolt.Tx), args[1].(store.Partition), args[2].(string), args[3].(uint32))
	})
	return _c
...
func (_c *MockStore_LoadShardVersion_Call) Return(_a0 uint64, _a1 error) *MockStore_LoadShardVersion_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *MockStore_LoadShardVersion_Call) RunAndReturn(run func(*bbolt.Tx, store.Partition, string, uint32) (uint64, error)) *MockStore_LoadShardVersion_Call {
	_c.Call.Return(run)
	return _c
}
