// Code generated by mockery. DO NOT EDIT.

package mocksymbolizer

import (
	context "context"
	io "io"

	mock "github.com/stretchr/testify/mock"
)

// MockDebugInfoStore is an autogenerated mock type for the DebugInfoStore type
type MockDebugInfoStore struct {
	mock.Mock
}

type MockDebugInfoStore_Expecter struct {
	mock *mock.Mock
}

func (_m *MockDebugInfoStore) EXPECT() *MockDebugInfoStore_Expecter {
	return &MockDebugInfoStore_Expecter{mock: &_m.Mock}
}

// Get provides a mock function with given fields: ctx, buildID
func (_m *MockDebugInfoStore) Get(ctx context.Context, buildID string) (io.ReadCloser, error) {
	ret := _m.Called(ctx, buildID)

	if len(ret) == 0 {
		panic("no return value specified for Get")
	}

	var r0 io.ReadCloser
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, string) (io.ReadCloser, error)); ok {
		return rf(ctx, buildID)
	}
	if rf, ok := ret.Get(0).(func(context.Context, string) io.ReadCloser); ok {
		r0 = rf(ctx, buildID)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(io.ReadCloser)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, string) error); ok {
		r1 = rf(ctx, buildID)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// MockDebugInfoStore_Get_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Get'
type MockDebugInfoStore_Get_Call struct {
	*mock.Call
}

// Get is a helper method to define mock.On call
//   - ctx context.Context
//   - buildID string
func (_e *MockDebugInfoStore_Expecter) Get(ctx interface{}, buildID interface{}) *MockDebugInfoStore_Get_Call {
	return &MockDebugInfoStore_Get_Call{Call: _e.mock.On("Get", ctx, buildID)}
}

func (_c *MockDebugInfoStore_Get_Call) Run(run func(ctx context.Context, buildID string)) *MockDebugInfoStore_Get_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(string))
	})
	return _c
}

func (_c *MockDebugInfoStore_Get_Call) Return(_a0 io.ReadCloser, _a1 error) *MockDebugInfoStore_Get_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *MockDebugInfoStore_Get_Call) RunAndReturn(run func(context.Context, string) (io.ReadCloser, error)) *MockDebugInfoStore_Get_Call {
	_c.Call.Return(run)
	return _c
}

// IsEnabled provides a mock function with given fields:
func (_m *MockDebugInfoStore) IsEnabled() bool {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for IsEnabled")
	}

	var r0 bool
	if rf, ok := ret.Get(0).(func() bool); ok {
		r0 = rf()
	} else {
		r0 = ret.Get(0).(bool)
	}

	return r0
}

// MockDebugInfoStore_IsEnabled_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'IsEnabled'
type MockDebugInfoStore_IsEnabled_Call struct {
	*mock.Call
}

// IsEnabled is a helper method to define mock.On call
func (_e *MockDebugInfoStore_Expecter) IsEnabled() *MockDebugInfoStore_IsEnabled_Call {
	return &MockDebugInfoStore_IsEnabled_Call{Call: _e.mock.On("IsEnabled")}
}

func (_c *MockDebugInfoStore_IsEnabled_Call) Run(run func()) *MockDebugInfoStore_IsEnabled_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *MockDebugInfoStore_IsEnabled_Call) Return(_a0 bool) *MockDebugInfoStore_IsEnabled_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockDebugInfoStore_IsEnabled_Call) RunAndReturn(run func() bool) *MockDebugInfoStore_IsEnabled_Call {
	_c.Call.Return(run)
	return _c
}

// Put provides a mock function with given fields: ctx, buildID, reader
func (_m *MockDebugInfoStore) Put(ctx context.Context, buildID string, reader io.Reader) error {
	ret := _m.Called(ctx, buildID, reader)

	if len(ret) == 0 {
		panic("no return value specified for Put")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, string, io.Reader) error); ok {
		r0 = rf(ctx, buildID, reader)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// MockDebugInfoStore_Put_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Put'
type MockDebugInfoStore_Put_Call struct {
	*mock.Call
}

// Put is a helper method to define mock.On call
//   - ctx context.Context
//   - buildID string
//   - reader io.Reader
func (_e *MockDebugInfoStore_Expecter) Put(ctx interface{}, buildID interface{}, reader interface{}) *MockDebugInfoStore_Put_Call {
	return &MockDebugInfoStore_Put_Call{Call: _e.mock.On("Put", ctx, buildID, reader)}
}

func (_c *MockDebugInfoStore_Put_Call) Run(run func(ctx context.Context, buildID string, reader io.Reader)) *MockDebugInfoStore_Put_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(string), args[2].(io.Reader))
	})
	return _c
}

func (_c *MockDebugInfoStore_Put_Call) Return(_a0 error) *MockDebugInfoStore_Put_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockDebugInfoStore_Put_Call) RunAndReturn(run func(context.Context, string, io.Reader) error) *MockDebugInfoStore_Put_Call {
	_c.Call.Return(run)
	return _c
}

// NewMockDebugInfoStore creates a new instance of MockDebugInfoStore. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewMockDebugInfoStore(t interface {
	mock.TestingT
	Cleanup(func())
}) *MockDebugInfoStore {
	mock := &MockDebugInfoStore{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
