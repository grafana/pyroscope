...
func TestSchedule_Update_LeaseRenewal(t *testing.T) {
	...
	t.Run("Owner", assertIdempotentSubtest(t, func(t *testing.T) {
	...
	t.Run("NotOwner", assertIdempotentSubtest(t, func(t *testing.T) {
		s := scheduler.NewSchedule(nil, &raft.Log{Index: 1, AppendedAt: time.Unix(0, 0)})
		update, err := s.UpdateJob(&metastorev1.CompactionJobStatusUpdate{
			Name:   "1",
			Token:  0,
	...
	t.Run("JobCompleted", assertIdempotentSubtest(t, func(t *testing.T) {
		s := scheduler.NewSchedule(nil, &raft.Log{Index: 1, AppendedAt: time.Unix(0, 0)})
		update, err := s.UpdateJob(&metastorev1.CompactionJobStatusUpdate{
			Name:   "0",
			Token:  1,
	...
	t.Run("WrongStatus", assertIdempotentSubtest(t, func(t *testing.T) {
		s := scheduler.NewSchedule(nil, &raft.Log{Index: 1, AppendedAt: time.Unix(0, 0)})
		update, err := s.UpdateJob(&metastorev1.CompactionJobStatusUpdate{
			Name:   "1",
			Token:  1,
...
// Test idempotency: the test must be valid when run multiple times.
