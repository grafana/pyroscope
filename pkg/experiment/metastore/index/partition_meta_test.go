...
func TestPartitionMeta_overlaps(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "simple overlap",
			meta: PartitionMeta{Timestamp: createTime("2024-09-11T06:00:00.000Z"), Duration: 6 * time.Hour},
			args: args{
		...
		},
		{
			...
			meta: PartitionMeta{Timestamp: createTime("2024-09-11T00:00:00.000Z"), Duration: 6 * time.Hour},
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equalf(t, tt.want, tt.meta.overlaps(tt.args.start, tt.args.end), "overlaps(%v, %v)", tt.args.start, tt.args.end)
		})
	}
}
