...
func (b *singleBlockQuerier) MergeByStacktraces(ctx context.Context, rows iter.Iterator[query.Profile]) (*ingestv1.MergeProfilesStacktracesResult, error) {
	stacktraceIDCol, _ := query.GetColumnIndexByPath(b.profiles.file, "Samples.list.element.StacktraceID")
	if stacktraceIDCol == -1 {
		return nil, fmt.Errorf("no stacktrace id column found")
	}
	valuesCol, _ := query.GetColumnIndexByPath(b.profiles.file, "Samples.list.element.Values.list.element")
	if valuesCol == -1 {
	...
	it := query.NewJoinIterator(
		...
		[]query.Iterator{
			...
			query.NewColumnIterator(context.Background(), b.profiles.file.RowGroups(), stacktraceIDCol, "Samples.list.element.StacktraceID", 1024, nil, "StacktraceID"),
			query.NewColumnIterator(context.Background(), b.profiles.file.RowGroups(), valuesCol, "Samples.list.element.Values.list.element", 10*1024, nil, "Value"),
		}, nil,
	)
	defer it.Close()
