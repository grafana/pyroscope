...
func (p *profileBuilder) convertLocationBack(ol *otelProfile.Location, dictionary *otelProfile.ProfilesDictionary) (uint64, error) {
	...
	if len(gl.Line) == 0 {
		filename, err := at(dictionary.StringTable, om.FilenameStrindex)
		if err != nil {
			return 0, fmt.Errorf("could not access location file name: %w", err)
		}
		gl.Line = append(gl.Line, &googleProfile.Line{
			FunctionId: p.addfunc(fmt.Sprintf("%s 0x%x", filename, ol.Address)),
		})
	}
