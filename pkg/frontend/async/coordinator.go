	...
	for {
		...
		case <-tickerC:
			if err := c.store.heartbeat(ctx, tenantID, requestID, &lease); errors.Is(err, errLostOwnership) {
				// Cancel the query this store no longer owns so it stops
				// pinning the concurrency slot -- unless it already finished:
				// a buffered result is a real outcome and must be reported
				// regardless of why ownership looks lost (see heartbeat's
				// ambiguous-ack comment).
				select {
				case res := <-resultCh:
					c.report(ctx, tenantID, requestID, &lease, res)
				default:
