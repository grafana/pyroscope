...
// wrapExpectedErrs keeps lost CAS races and routine not-found checks out of
// bucket failure metrics: objectExists/specAbsent run on every poll and
// adoption attempt, so a not-found there is normal outcome, not a bucket
// problem. Both instrumented-bucket interfaces are checked because
// pkg/objstore wraps thanos buckets with its own.
func wrapExpectedErrs(bucket objstore.Bucket) objstore.Bucket {
	expected := func(err error) bool {
		return bucket.IsConditionNotMetErr(err) || bucket.IsObjNotFoundErr(err)
	}
	if ib, ok := bucket.(phlareobj.InstrumentedBucket); ok {
		return ib.WithExpectedErrs(expected)
	}
