...
func (q *headInMemoryQuerier) MergeByLabels(
	...
	if len(sts.GetCallSite()) == 0 {
		for rows.Next() {
			...
			seriesBuilder.Add(p.Fingerprint(), p.Labels(), int64(p.Timestamp()), float64(p.Total()), nil)
		}
	} else {
		r := symdb.NewResolver(ctx, q.head.symdb,
			symdb.WithResolverStackTraceSelector(sts))
		...
		for rows.Next() {
			...
			seriesBuilder.Add(p.Fingerprint(), p.Labels(), int64(p.Timestamp()), float64(v.Total), nil)
		}
	}
	...
	if len(sts.GetCallSite()) == 0 {
		for _, fp := range ids {
			...
			for _, p := range profileSeries.profiles {
				...
				seriesBuilder.Add(fp, profileSeries.lbs, int64(p.Timestamp()), float64(p.Total()), nil)
			}
		}
	} else {
		r := symdb.NewResolver(ctx, q.head.symdb,
		...
		for _, fp := range ids {
			...
			for _, p := range profileSeries.profiles {
				...
				seriesBuilder.Add(fp, profileSeries.lbs, int64(p.Timestamp()), float64(v.Total), nil)
			}
		}
	}
	return seriesBuilder.Build(), nil
