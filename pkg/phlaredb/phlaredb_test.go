...
func ingestProfiles(b testing.TB, db *PhlareDB, generator func(tsNano int64, t testing.TB) (*googlev1.Profile, string), from, to int64, step time.Duration, externalLabels ...*typesv1alpha1.LabelPair) {
	b.Helper()
	for i := from; i <= to; i += int64(step) {
		p, name := generator(i, b)
		require.NoError(b, db.Head().Ingest(
			context.Background(), p, uuid.New(), append(externalLabels, &typesv1alpha1.LabelPair{Name: model.MetricNameLabel, Value: name})...))
	}
}

type fakeBidiServerMergeProfilesStacktraces struct {
	profilesSent []*ingestv1alpha1.ProfileSets
	keep         [][]bool
	t            *testing.T
}

func (f *fakeBidiServerMergeProfilesStacktraces) Send(resp *ingestv1alpha1.MergeProfilesStacktracesResponse) error {
	f.profilesSent = append(f.profilesSent, testhelper.CloneProto(f.t, resp.SelectedProfiles))
	return nil
}

func (f *fakeBidiServerMergeProfilesStacktraces) Receive() (*ingestv1alpha1.MergeProfilesStacktracesRequest, error) {
	res := &ingestv1alpha1.MergeProfilesStacktracesRequest{
		Profiles: f.keep[0],
	}
	f.keep = f.keep[1:]
	return res, nil
...
func (f *PhlareDB) ingesterClient() (ingesterv1alpha1connect.IngesterServiceClient, func()) {
	mux := http.NewServeMux()
	mux.Handle(ingesterv1alpha1connect.NewIngesterServiceHandler(&ingesterHandlerPhlareDB{f}))
	serv := testhelper.NewInMemoryServer(mux)

	var httpClient *http.Client = serv.Client()

	client := ingesterv1alpha1connect.NewIngesterServiceClient(
		httpClient, serv.URL(),
	)
...
func (i *ingesterHandlerPhlareDB) Push(context.Context, *connect.Request[pushv1alpha1.PushRequest]) (*connect.Response[pushv1alpha1.PushResponse], error) {
	return nil, errors.New("not implemented")
}

func (i *ingesterHandlerPhlareDB) LabelValues(context.Context, *connect.Request[ingestv1alpha1.LabelValuesRequest]) (*connect.Response[ingestv1alpha1.LabelValuesResponse], error) {
	return nil, errors.New("not implemented")
}

func (i *ingesterHandlerPhlareDB) LabelNames(context.Context, *connect.Request[ingestv1alpha1.LabelNamesRequest]) (*connect.Response[ingestv1alpha1.LabelNamesResponse], error) {
	return nil, errors.New("not implemented")
}

func (i *ingesterHandlerPhlareDB) ProfileTypes(context.Context, *connect.Request[ingestv1alpha1.ProfileTypesRequest]) (*connect.Response[ingestv1alpha1.ProfileTypesResponse], error) {
	return nil, errors.New("not implemented")
}

func (i *ingesterHandlerPhlareDB) Series(context.Context, *connect.Request[ingestv1alpha1.SeriesRequest]) (*connect.Response[ingestv1alpha1.SeriesResponse], error) {
	return nil, errors.New("not implemented")
}

func (i *ingesterHandlerPhlareDB) Flush(context.Context, *connect.Request[ingestv1alpha1.FlushRequest]) (*connect.Response[ingestv1alpha1.FlushResponse], error) {
	return nil, errors.New("not implemented")
}
...
func TestMergeProfilesStacktraces(t *testing.T) {
	...
	ingestProfiles(t, db, cpuProfileGenerator, start.UnixNano(), end.UnixNano(), step,
		&typesv1alpha1.LabelPair{Name: "namespace", Value: "my-namespace"},
		&typesv1alpha1.LabelPair{Name: "pod", Value: "my-pod"},
	)
	...
	t.Run("request the one existing series", func(t *testing.T) {
		...
		require.NoError(t, bidi.Send(&ingestv1alpha1.MergeProfilesStacktracesRequest{
...
func TestFilterProfiles(t *testing.T) {
	...
	filtered, err := filterProfiles[
		...
		*ingestv1alpha1.MergeProfilesStacktracesRequest](ctx, in, 5, bidi)
	require.NoError(t, err)
	require.Equal(t, 2, len(filtered))
	require.Equal(t, 3, len(bidi.profilesSent))
	testhelper.EqualProto(t, []*ingestv1alpha1.ProfileSets{
		{
			LabelsSets: lo.Times(5, func(i int) *typesv1alpha1.Labels {
				return &typesv1alpha1.Labels{Labels: phlaremodel.LabelsFromStrings("foo", "bar", "i", fmt.Sprintf("%d", i))}
			}),
			Profiles: lo.Times(5, func(i int) *ingestv1alpha1.SeriesProfile {
				return &ingestv1alpha1.SeriesProfile{Timestamp: int64(model.TimeFromUnixNano(int64(i * int(time.Minute)))), LabelIndex: int32(i)}
			}),
		},
		{
			LabelsSets: lo.Times(5, func(i int) *typesv1alpha1.Labels {
				return &typesv1alpha1.Labels{Labels: phlaremodel.LabelsFromStrings("foo", "bar", "i", fmt.Sprintf("%d", i+5))}
			}),
			Profiles: lo.Times(5, func(i int) *ingestv1alpha1.SeriesProfile {
				return &ingestv1alpha1.SeriesProfile{Timestamp: int64(model.TimeFromUnixNano(int64((i + 5) * int(time.Minute)))), LabelIndex: int32(i)}
			}),
		},
		{
			LabelsSets: lo.Times(1, func(i int) *typesv1alpha1.Labels {
				return &typesv1alpha1.Labels{Labels: phlaremodel.LabelsFromStrings("foo", "bar", "i", fmt.Sprintf("%d", i+10))}
			}),
			Profiles: lo.Times(1, func(i int) *ingestv1alpha1.SeriesProfile {
				return &ingestv1alpha1.SeriesProfile{Timestamp: int64(model.TimeFromUnixNano(int64((i + 10) * int(time.Minute)))), LabelIndex: int32(i)}
			}),
		},
	}, bidi.profilesSent)
