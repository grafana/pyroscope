...
func TestMergePprof(t *testing.T) {
	...
	q.queriers[0].Sort(profiles)
	result, err := q.queriers[0].MergePprof(ctx, iter.NewSliceIterator(profiles), 0, phlaremodel.FunctionSelector{})
	require.NoError(t, err)
...
func TestHeadMergePprof(t *testing.T) {
	...
	db.headQueriers()[0].Sort(profiles)
	result, err := db.headQueriers()[0].MergePprof(ctx, iter.NewSliceIterator(profiles), 0, phlaremodel.FunctionSelector{})
	require.NoError(t, err)
