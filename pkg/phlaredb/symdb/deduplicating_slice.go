...
func (p *Partition) convertSamples(r *rewriter, in []*profilev1.Sample) []schemav1.Samples {
	...
	p.AppendStacktraces(stacktracesIds, stacktraces)
...
type deduplicatingSlice[M schemav1.Models, K comparable, H Helper[M, K], P schemav1.Persister[M]] struct {
	slice  []M
	size   atomic.Uint64
	lock   sync.RWMutex
	lookup map[K]int64
...
func (s *deduplicatingSlice[M, K, H, P]) ingest(elems []M, rewriter *rewriter) {
	var (
