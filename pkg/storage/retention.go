...
func (s *Storage) EnforceRetentionPolicy(rp *segment.RetentionPolicy) error {
	...
	err := s.iterateOverAllSegments(func(k *flameql.Key) error {
...
func (s *Storage) deleteSegmentData(k *flameql.Key, rp *segment.RetentionPolicy) error {
	sk := k.SegmentKey()
	...
	for _, n := range nodes {
		treeKey := flameql.TreeKey(sk, n.depth, n.time)
...
func (s *Storage) reclaimSegmentSpace(k *flameql.Key, size int64) error {
	batchSize := s.trees.MaxBatchCount()
	batch := s.trees.NewWriteBatch()
	defer func() {
		batch.Cancel()
	...
	err = s.trees.View(func(txn *badger.Txn) error {
		...
		for it.Rewind(); it.Valid(); it.Next() {
			...
			if tk, ok := treePrefix.trim(item.Key()); ok {
				...
				t, level, err := flameql.ParseTreeKey(treeKey)
				if err == nil {
...
func (s *Storage) iterateOverAllSegments(cb func(*flameql.Key) error) error {
	nameKey := "__name__"
	...
	for _, r := range dimension.Union(dimensions...) {
		k, err := flameql.ParseKey(string(r))
		if err != nil {
