...
func (c *RecorderConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.Int64Var(&c.MaxRetainedBytes, "profile-dump.max-retained-bytes", d.MaxRetainedBytes, "Local recorder byte budget covering preparation, envelope buffers, serialization scratch and uploads. Provisional development default.")
...
func (c RecorderConfig) Validate() error {
	if c.MaxObjectBytes <= int64(HeaderSize) || c.MaxObjectBytes > int64(math.MaxInt)-encodingReservation || c.MaxRetainedBytes < c.MaxObjectBytes+encodingReservation {
		return fmt.Errorf("recorder object size must fit an int and retained bytes must cover a maximum object plus %d bytes encoding overhead", encodingReservation)
	}
