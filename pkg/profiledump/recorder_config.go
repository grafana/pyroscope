...
func (c *RecorderConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.Int64Var(&c.MaxRetainedBytes, "profile-dump.max-retained-bytes", d.MaxRetainedBytes, "Per-distributor budget for admitted capture buffers, including metadata overlap, item overhead, queueing and uploads. Initial bounded metadata marshaling and provider allocations are outside this budget. This is not an RSS limit. Provisional development default.")
	...
	f.DurationVar(&c.UploadTimeout, "profile-dump.upload-timeout", d.UploadTimeout, "Cooperative timeout for each background capture upload. Providers that ignore cancellation may exceed it. Provisional development default.")
