...
// Compile creates an immutable policy using now to bound the activation window.
...
func (c *TenantConfig) Compile(bounds Config, now time.Time) (Policy, error) {
	...
	matchers, selector, err := compileSelector(selector)
	if err != nil {
		return Policy{}, err
	}
	deadline := c.ActiveUntil.UTC()
	// Versioned, newline-separated scalars. Selector strings escape newlines.
...
func compileSelector(selector string) ([]*labels.Matcher, string, error) {
	matchers, err := model.ParseMetricSelector(selector)
	if err != nil {
		return nil, "", fmt.Errorf("invalid selector: %w", err)
	}
	// Canonical matcher strings stabilize the fingerprint without regex equivalence analysis.
	parts := make([]string, len(matchers))
	for i, matcher := range matchers {
		parts[i] = matcher.String()
	}
	slices.Sort(parts)
	return matchers, "{" + strings.Join(slices.Compact(parts), ",") + "}", nil
}
