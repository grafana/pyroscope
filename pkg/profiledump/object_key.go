...
func (f Format) valid() bool { return f == FormatPprof }
...
// NewObjectKey returns a key and capture ID using server time and a random ULID.
...
func ParseObjectKey(key string) (ObjectKey, error) {
	...
	const maxKeyBytes = len(ObjectPrefix) + (MaxTenantBytes+2)/3*4 + 1 + len("2006/01/02/") + 26 + 1 + len(FormatPprof) + len(".pyrdump")
