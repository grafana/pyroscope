...
func testMetadata(size int64) Metadata {
	...
	return Metadata{
		...
		SourceProtocol: SourceConnect, NativeFormat: FormatPprof,
		Incoming:          &Representation{ContentType: "application/protobuf", Encoding: "gzip", Syntax: "protobuf"},
		Stored:            Representation{ContentType: "application/protobuf", Encoding: "gzip", Syntax: "protobuf"},
...
func TestEnvelopeRoundTrip(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"compressed pprof", SourceConnect, FormatPprof, Representation{"application/protobuf", "gzip", "protobuf"}, []byte{0x1f, 0x8b, 0x08, 0, 0xff, 0, 42}},
		{"JFR", SourceIngest, FormatJFR, Representation{"application/octet-stream", "identity", "binary"}, []byte{'F', 'L', 'R', 0, 0xff}},
		{"multipart JFR", SourceIngest, FormatJFR, Representation{`multipart/form-data; boundary="capture-boundary"`, "identity", "multipart"}, []byte("--capture-boundary\r\nContent-Disposition: form-data; name=\"jfr\"\r\n\r\nFLR\x00\xff\r\n--capture-boundary--\r\n")},
		{"legacy lines", SourceIngest, FormatLines, Representation{"text/plain; charset=utf-8", "identity", "text"}, []byte("main;work 42\n")},
		{"OTLP decoded JSON", SourceOTLPHTTP, FormatOTLP, Representation{"application/x-protobuf", "identity", "protobuf"}, []byte{0x0a, 0x02, 0x08, 0x01}},
		{"OTLP failed JSON", SourceOTLPHTTP, FormatOTLP, Representation{"application/json", "identity", "json"}, []byte(`{"resourceProfiles": [`)},
		{"OTLP gRPC", SourceOTLPGRPC, FormatOTLP, Representation{"application/x-protobuf", "identity", "protobuf"}, []byte{0x0a, 0x00}},
		{"empty", SourceConnect, FormatPprof, Representation{"application/octet-stream", "identity", "binary"}, []byte{}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			m := testMetadata(int64(len(tc.payload)))
			m.SourceProtocol, m.NativeFormat, m.Stored = tc.source, tc.format, tc.stored
			if tc.source == SourceOTLPHTTP {
				m.Incoming = &Representation{"application/json", "gzip", "json"}
			}
			if tc.source == SourceOTLPGRPC {
				m.Incoming = nil
			}
			...
			got, err := Decode(bytes.NewReader(encoded), &restored, int64(len(encoded)))
			require.NoError(t, err)
			require.Equal(t, m, got)
			require.True(t, bytes.Equal(tc.payload, restored.Bytes()))
			require.Equal(t, tc.payload, encoded[14+int(metadataLength):])
		})
	}
}
...
func TestEnvelopeInvalidMetadata(t *testing.T) {
	...
	for name, mutate := range map[string]func(*Metadata){
		...
		"invalid incoming":                func(m *Metadata) { m.Incoming.ContentType = ";" },
		"content type size":               func(m *Metadata) { m.Stored.ContentType = strings.Repeat("a", MaxTextBytes+1) },
		"encoding":                        func(m *Metadata) { m.Stored.Encoding = "invalid-encoding" },
		"syntax":                          func(m *Metadata) { m.Stored.Syntax = "invalid-syntax" },
		"boundary missing":                func(m *Metadata) { m.Stored = Representation{"multipart/form-data", "identity", "multipart"} },
		"multipart syntax mismatch":       func(m *Metadata) { m.Stored = Representation{"multipart/form-data; boundary=x", "identity", "binary"} },
		"multipart content type mismatch": func(m *Metadata) { m.Stored.Syntax = "multipart" },
...
func TestMetadataWireBound(t *testing.T) {
	...
	// Individually valid fields can exceed the wire limit after JSON escaping.
	m := testMetadata(0)
	...
	m.Stored.ContentType = `application/octet-stream; name="` + strings.Repeat("<", 900) + `"`
	m.Incoming = &m.Stored
	require.NoError(t, m.Validate())
	raw, err = json.Marshal(m)
	require.NoError(t, err)
	require.Greater(t, len(raw), MaxMetadataSize)
	var output bytes.Buffer
	require.Error(t, Encode(&output, m, bytes.NewReader(nil), math.MaxInt64))
	require.Empty(t, output.Bytes())
}
