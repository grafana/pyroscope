...
func (c *CleanerConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.DurationVar(&c.SweepTimeout, "profile-dump.sweep-timeout", d.SweepTimeout, "Time budget per admin capture cleanup pass and maximum wait for a listing to produce its next entry. Paused listings resume on later passes. Must be positive. Provisional development default.")
	f.IntVar(&c.MaxEntries, "profile-dump.cleanup-max-entries", d.MaxEntries, "Maximum listing entries processed per cleanup pass. Listings pause at the budget and resume without replay. Must be positive. Provisional development default.")
}
