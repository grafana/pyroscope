...
func TestObjectKeyRoundTrip(t *testing.T) {
	t.Parallel()
	for _, format := range []Format{FormatPprof} {
...
func TestObjectKeyFreshIDs(t *testing.T) {
	...
	key1, id1, err := newObjectKey("a", capturedAt, FormatPprof, entropy)
	require.NoError(t, err)
	key2, id2, err := newObjectKey("a", capturedAt, FormatPprof, entropy)
	require.NoError(t, err)
	require.NotEqual(t, id1, id2)
	require.NotEqual(t, key1, key2)
	require.Equal(t, id1.Time(), id2.Time())
	_, _, err = newObjectKey("a", capturedAt, FormatPprof, entropy)
	require.Error(t, err, "entropy failures propagate")
	// Exercise the production entropy source without probabilistic assertions.
	key, id, err := NewObjectKey("a", capturedAt, FormatPprof)
	require.NoError(t, err)
	parsed, err := ParseObjectKey(key)
	require.NoError(t, err)
	require.Equal(t, id, parsed.CaptureID)
	_, _, err = NewObjectKey("", capturedAt, FormatPprof)
	require.Error(t, err)
