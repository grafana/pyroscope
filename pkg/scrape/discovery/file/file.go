...
func (d *Discovery) listFiles() []string {
	...
	for _, p := range d.paths {
		...
		if err != nil {
			d.logger.WithError(err).WithField("glob", p).Errorf("expanding glob")
			continue
		}
...
func (d *Discovery) watchFiles() {
	...
	for _, p := range d.paths {
		...
		if err := d.watcher.Add(p); err != nil {
			d.logger.WithField("path", p).WithError(err).Errorf("adding file watch")
		}
	}
}
...
func (d *Discovery) Run(ctx context.Context, ch chan<- []*targetgroup.Group) {
	...
	if err != nil {
		d.logger.WithError(err).Errorf("adding file watch")
		return
	}
	d.watcher = watcher
	defer d.stop()
	...
	for {
		...
		case err = <-d.watcher.Errors:
			if err != nil {
				d.logger.WithError(err).Errorf("watching file")
			}
		}
	}
}
...
func (d *Discovery) stop() {
	d.logger.WithField("paths", fmt.Sprintf("%v", d.paths)).Debugf("stopping file discovery")
	...
	if err := d.watcher.Close(); err != nil {
		d.logger.WithError(err).
			WithField("paths", fmt.Sprintf("%v", d.paths)).
...
func (d *Discovery) refresh(ctx context.Context, ch chan<- []*targetgroup.Group) {
	...
	for _, p := range d.listFiles() {
		...
		if err != nil {
			...
			d.logger.WithError(err).WithField("path", p).Error("reading file")
	...
	for f, n := range d.lastRefresh {
		m, ok := ref[f]
		if !ok || n > m {
			d.logger.WithField("file", f).Debug("file_sd refresh found file that should be removed")
			d.deleteTimestamp(f)
			for i := m; i < n; i++ {
				select {
