...
func TestRecordingRules_Get(t *testing.T) {
	...
	t.Run("Get not found", func(t *testing.T) {
		_, err := r.RecordingRules.GetRecordingRule(ctx, connect.NewRequest(&settingsv1.GetRecordingRuleRequest{Id: storeRule1.Id}))
	...
	t.Run("List rules empty for other user", func(t *testing.T) {
		...
		resp, err := r.RecordingRules.ListRecordingRules(ctx2, connect.NewRequest(&settingsv1.ListRecordingRulesRequest{}))
		require.NoError(t, err)
	...
	t.Run("Get config rule from autogenerated ID", func(t *testing.T) {
		...
		resp, err := r.RecordingRules.GetRecordingRule(ctx, connect.NewRequest(&settingsv1.GetRecordingRuleRequest{Id: idForConfigRule2}))
		require.NoError(t, err)
		require.Equal(t, configRule2, resp.Msg.Rule)
	})

	t.Run("Insert store rules", func(t *testing.T) {
		rule, err := r.RecordingRules.UpsertRecordingRule(ctx, connect.NewRequest(&settingsv1.UpsertRecordingRuleRequest{
			Id:               storeRule1.Id,
	...
	t.Run("Get overridden rule from config", func(t *testing.T) {
		resp, err := r.RecordingRules.GetRecordingRule(ctx, connect.NewRequest(&settingsv1.GetRecordingRuleRequest{Id: storeRule2.Id}))
		require.NoError(t, err)
	...
	t.Run("List rules with overrides", func(t *testing.T) {
		resp, err := r.RecordingRules.ListRecordingRules(ctx, connect.NewRequest(&settingsv1.ListRecordingRulesRequest{}))
		require.NoError(t, err)
	...
	t.Run("Upsert overridden rule just changes the original", func(t *testing.T) {
		upsertedRule, err := r.RecordingRules.UpsertRecordingRule(ctx, connect.NewRequest(&settingsv1.UpsertRecordingRuleRequest{
			Id:               storeRule2.Id,
			MetricName:       storeRule2.MetricName,
			Matchers:         storeRule2.Matchers,
			GroupBy:          storeRule2.GroupBy,
		...
		rule, err := r.RecordingRules.GetRecordingRule(ctx, connect.NewRequest(&settingsv1.GetRecordingRuleRequest{Id: storeRule2.Id}))
		require.NoError(t, err)
		require.Equal(t, configRule1, rule.Msg.Rule)
	})

	t.Run("Delete store rules", func(t *testing.T) {
		_, err := r.RecordingRules.DeleteRecordingRule(ctx, connect.NewRequest(&settingsv1.DeleteRecordingRuleRequest{Id: storeRule1.Id}))
		require.NoError(t, err)
		_, err = r.RecordingRules.DeleteRecordingRule(ctx, connect.NewRequest(&settingsv1.DeleteRecordingRuleRequest{Id: storeRule2.Id}))
		require.NoError(t, err)
		resp, err := r.RecordingRules.ListRecordingRules(ctx, connect.NewRequest(&settingsv1.ListRecordingRulesRequest{}))
		require.NoError(t, err)
		require.EqualValues(t, resp.Msg.Rules, []*settingsv1.RecordingRule{
	...
	t.Run("Can't delete config rules", func(t *testing.T) {
		_, err := r.RecordingRules.DeleteRecordingRule(ctx, connect.NewRequest(&settingsv1.DeleteRecordingRuleRequest{Id: configRule1.Id}))
