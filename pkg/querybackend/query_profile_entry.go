...
func profileEntryIterator(q *queryContext) (iter.Iterator[ProfileEntry], error) {
	series, err := getSeries(q.ds.Index(), q.req.matchers)
	if err != nil {
		return nil, err
	}
...
func getSeries(reader phlaredb.IndexReader, matchers []*labels.Matcher) (map[uint32]series, error) {
