...
func TestPushBatch_InvalidProfileIDToSegmentWriter(t *testing.T) {
	writer := new(recordingSegmentWriter)
	d := newProbeDistributor(t, writer, 1) // Deterministic generation is disabled.
	series := probeProfileSeries(1)
	series[0].ID = "not-a-uuid"

	err := d.PushBatch(tenant.InjectTenantID(t.Context(), "user-1"), &distributormodel.PushRequest{
		RawProfileType: distributormodel.RawProfileTypePPROF,
		Series:         series,
	})
	require.NoError(t, err)
	require.NotNil(t, writer.request)
	id, err := uuid.FromBytes(writer.request.ProfileId)
	require.NoError(t, err)
