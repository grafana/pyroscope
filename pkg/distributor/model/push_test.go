...
func TestMarkThrottledTenant(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			verify: func(t *testing.T, req *PushRequest) {
				require.Len(t, req.Series2.Annotations, 1)
				assert.Equal(t, ingestlimits.ProfileAnnotationKeyThrottled, req.Series2.Annotations[0].Key)
				assert.Contains(t, req.Series2.Annotations[0].Value, "\"periodLimitMb\":128")
			},
		},
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			verify: func(t *testing.T, req *PushRequest) {
				require.Len(t, req.Series2.Annotations, 1)
				assert.Equal(t, ingestlimits.ProfileAnnotationKeyThrottled, req.Series2.Annotations[0].Key)
				assert.Contains(t, req.Series2.Annotations[0].Value, "\"periodLimitMb\":64")
				assert.Contains(t, req.Series2.Annotations[0].Value, "\"usageGroup\":\"group-1\"")
			},
		},
		{
