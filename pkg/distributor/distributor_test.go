...
func TestPushParsed_ServiceNameHandling(t *testing.T) {
	ingesterClient := newFakeIngester(t, false)
	d, err := New(
		Config{DistributorRing: ringConfig},
		testhelper.NewMockRing([]ring.InstanceDesc{{Addr: "foo"}}, 3),
	...
	testCases := []struct {
		...
		{
			name: "with service_name label",
			inputLabels: []*typesv1.LabelPair{
				{Name: "cluster", Value: "us-central1"},
				{Name: "__name__", Value: "cpu"},
				{Name: phlaremodel.LabelNameServiceName, Value: "myservice"},
			},
			expectedLabels: map[string]string{
				"cluster":                        "us-central1",
				"__name__":                       "cpu",
				phlaremodel.LabelNameServiceName: "myservice",
			},
		},
		{
			name: "with service.name label",
			inputLabels: []*typesv1.LabelPair{
				{Name: "cluster", Value: "us-central1"},
				{Name: "__name__", Value: "cpu"},
				{Name: "service.name", Value: "myservice"},
			},
			expectedLabels: map[string]string{
				"cluster":                        "us-central1",
				"__name__":                       "cpu",
		...
		{
			name: "with both service_name and service.name, otel one takes precedence",
			inputLabels: []*typesv1.LabelPair{
				{Name: "cluster", Value: "us-central1"},
				{Name: "__name__", Value: "cpu"},
				{Name: phlaremodel.LabelNameServiceName, Value: "primary"},
				{Name: "service.name", Value: "secondary"},
			},
			expectedLabels: map[string]string{
				"cluster":                        "us-central1",
				"__name__":                       "cpu",
				phlaremodel.LabelNameServiceName: "secondary",
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := d.PushParsed(ctx, req)
			require.NoError(t, err)

			// Verify the labels were processed correctly
			require.NotEmpty(t, ingesterClient.requests)
			require.NotEmpty(t, ingesterClient.requests[0].Series)

			// Convert labels to a map for easier verification
			actualLabels := make(map[string]string)
			for _, label := range ingesterClient.requests[0].Series[0].Labels {
				actualLabels[label.Name] = label.Value
			}

			// Check that all expected labels are present
			for name, value := range tc.expectedLabels {
				require.Equal(t, value, actualLabels[name], "Label %s should have value %s", name, value)
			}

			// Check that service_name label is present
			_, hasServiceName := actualLabels[phlaremodel.LabelNameServiceName]
			require.True(t, hasServiceName, "service_name label should be present")
		})
	}
}
