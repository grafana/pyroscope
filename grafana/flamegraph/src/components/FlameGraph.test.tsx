...
describe('FlameGraph', () => {
  ...
  it('should render children', async () => {
    ...
    await act(async () => {
      ...
      expect(screen.getByTestId('flamegraph').children[0].textContent).toEqual('total');
      expect(screen.getByTestId('flamegraph').children[1].textContent).toEqual('github.com/grafana/fire/pkg/distributor.(*Distributor).Push.func1');
    ...
    await act(async () => {
      ...
      expect(screen.getByTestId('flamegraph').children[3].textContent).toEqual('github.com/grafana/fire/pkg/agent.(*Target).start.func1');
    ...
    await act(async () => {
      ...
      expect(screen.getByTestId('flamegraph').children[20].textContent).toEqual('text/template/parse.(*PipeNode).append');
      await userEvent.click(screen.getByTestId('flamegraph').children[20]);
      ...
      expect(screen.getByTestId('flamegraph').children[19].textContent).toEqual('text/template/parse.(*PipeNode).append');
    });
    ...
    await act(async () => {
      ...
      expect(screen.getByTestId('flamegraph').children.length).toEqual(21)
      expect(screen.getByTestId('flamegraph').children[19].textContent).toEqual('text/template/parse.(*Tree).newPipeline');
      expect(screen.getByTestId('flamegraph').children[20].textContent).toEqual('text/template/parse.(*PipeNode).append');
      ...
      await userEvent.click(screen.getByTestId('flamegraph').children[4]);
      expect(screen.getByTestId('flamegraph').children.length).toEqual(108)
      expect(screen.getByTestId('flamegraph').children[19].textContent).toEqual('github.com/prometheus/client_golang/prometheus.(*SummaryVec).WithLabelValues');
      expect(screen.getByTestId('flamegraph').children[20].textContent).toEqual('text/template/parse.(*Tree).Parse');
      await userEvent.click(screen.getByTestId('flamegraph').children[3]);
      expect(screen.getByTestId('flamegraph').children.length).toEqual(108)
      expect(screen.getByTestId('flamegraph').children[19].textContent).toEqual('github.com/prometheus/client_golang/prometheus.(*SummaryVec).WithLabelValues');
