...
func addQueryBlocksParams(queryCmd commander) *queryBlocksParams {
	...
	queryCmd.Flag("block-tenant-id", "Tenant id of the queried block for remote bucket").StringVar(&params.BlockTenantID)
	return params
}

func queryBlocksMerge(ctx context.Context, params *queryMergeParams) error {
	level.Info(logger).Log("msg", "query-block merge", "blockIds", fmt.Sprintf("%v", params.BlockIds), "localPath",
		params.LocalPath, "bucketName", params.BucketName, "blockTenantId", params.BlockTenantID, "query", params.Query, "type", params.ProfileType)
	...
	stackTraceSelectors := getStackTraceSelector(params)
...
func queryBlocksSeries(ctx context.Context, params *querySeriesParams) error {
	level.Info(logger).Log("msg", "query-block series", "labelNames", fmt.Sprintf("%v", params.LabelNames),
		"blockIds", fmt.Sprintf("%v", params.BlockIds), "localPath", params.LocalPath, "bucketName", params.BucketName, "tenantId", params.BlockTenantID)
...
func getRemoteBucket(ctx context.Context, params *queryBlocksParams) (objstore.Bucket, error) {
	...
	return objstoreclient.NewBucket(ctx, objstoreclient.Config{
		...
		StoragePrefix: fmt.Sprintf("%s/phlaredb", params.BlockTenantID),
	}, params.BucketName)
}
