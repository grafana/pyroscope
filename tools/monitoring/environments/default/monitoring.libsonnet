local pyroscope = import './../../../../operations/pyroscope/jsonnet/pyroscope-mixin/pyroscope-mixin/mixin.libsonnet';
...
local fqdn = {
  pyroscope: 'pyroscope-micro-services-query-frontend.default.svc.cluster.local.',
};
...
prometheus {
  ...
  grafana_datasource_config_map+: k.core.v1.configMap.withDataMixin({
    'pyroscope-datasource.yml': k.util.manifestYaml({
      apiVersion: 1,
      datasources: [
        prom_k_grafana.grafana_datasource(
          'Pyroscope',
          'http://%s:4040' % fqdn.pyroscope,
