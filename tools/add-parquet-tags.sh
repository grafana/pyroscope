...
gomodifytags -file "${ROOT}/pkg/gen/google/v1/profile.pb.go" -struct Profile -add-tags parquet -template "-" -w -quiet

# Profile
gomodifytags -file "${ROOT}/pkg/gen/google/v1/profile.pb.go" -override -struct Profile -field TimeNanos -add-tags parquet -template ",delta" -w -quiet 

for f in SampleType Sample Mapping Location Function StringTable; do 
    gomodifytags -file "${ROOT}/pkg/gen/google/v1/profile.pb.go" -struct Profile -field "${f}" -add-tags parquet -template "," -w -quiet -override
done
