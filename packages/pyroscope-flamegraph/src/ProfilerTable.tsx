...
function generateCellDouble(
  ...
  c.totalLeft =
    zero(c.totalLeft) + (addTotal ? ff.getBarTotalLeft(level, j) : 0);
  c.totalRght =
    zero(c.totalRght) + (addTotal ? ff.getBarTotalRght(level, j) : 0);
  c.totalDiff =
    zero(c.totalDiff) + (addTotal ? ff.getBarTotalDiff(level, j) : 0);
  return c;
}
