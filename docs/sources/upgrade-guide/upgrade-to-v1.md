...
This strategic shift has been in the works for some time as we transition away from CLI-based profiling towards embracing native integrations. Moving forward, we encourage users to take advantage of native integrations tailored to specific programming languages, such as pip packages for Python, .NET packages for .NET applications, ruby gems for Ruby applications, and so on. Our [eBPP integration](/docs/pyroscope/latest/configure-client/grafana-agent/ebpf/) is also a good way to get profiling data for your whole cluster.
...
* Migrate your configuration from the old format to the new format (old config is usually located at `/etc/pyroscope/server.yml` and the new config is at `/etc/pyroscope/config.yaml`). There's a detailed description of all configuration parameters [here](/docs/pyroscope/latest/configure-server/reference-configuration-parameters/).
...
* Install the new helm chart:
  ...
  ```
  For more information on how to install the Helm chart, see our Helm documentation [here](/docs/pyroscope/latest/deploy-kubernetes/).
