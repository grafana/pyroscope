...
Grafana Fire's [ingester] component processes the received profiling data.
...
`-firedb.max-block-duration` (by default 3 hours), it will write the block to
...
Grafana Fire's data path `-firedb.data-path=` (by default
...
To avoid loosing the most recent data, Grafana Fire will remove the oldest
...
```
level=warn caller=firedb.go:231 ts=2022-10-05T13:19:09.770693308Z msg="disk utilization is high, deleted oldest block" path=data/anonymous/local/01GDZYHKKKY2ANY6PCJJZGT1N8
```
