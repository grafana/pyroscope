---
aliases:
  - /docs/fire/latest/operators-guide/securing/authentication-and-authorization/
description: Learn how to configure and run Grafana Fire with multi-tenancy.
menuTitle: Authentication and authorization
title: Grafana Fire authentication and authorization
weight: 20
---

# Grafana Fire authentication and authorization

Grafana Fire is a multi-tenant system where tenants can query profiles and alerts that include their tenant ID.
...
To protect Grafana Fire from accidental or malicious calls, you must add a layer of protection such as a reverse proxy that authenticates requests and injects the appropriate tenant ID into the `X-Scope-OrgID` header.
...
In trusted environments where you want to split series on Prometheus labels, you can run [cortex-tenant](https://github.com/blind-oracle/cortex-tenant) between a Prometheus server and Grafana Fire.

> **Note:** cortex-tenant is a third-party community project that is not maintained by Grafana Labs.

When proxying the timeseries to Grafana Fire, you can configure cortex-tenant to use specified labels as the `X-Scope-OrgID` header.
...
To disable multi-tenant functionality, pass the following argument to every Grafana Fire component:

`-auth.multitenancy-enabled=false`

After you disable multi-tenancy, Grafana Fire components internally set the tenant ID to the string `anonymous` for every request.
