---
title: "Grafana Fire hash rings"
menuTitle: "Hash rings"
description: "Hash rings distribute sharding and replication work among Grafana Fire components."
weight: 60
---

# Grafana Fire hash rings

Hash rings are a distributed [consistent hashing scheme](https://en.wikipedia.org/wiki/Consistent_hashing) and are widely used by Grafana Fire for sharding and replication.

## How the hash ring works in Grafana Fire

The hash ring in Grafana Fire is used to share work across several replicas of a component in a consistent way, so that any other component can decide which address to talk to.
The workload or data to share is hashed first and the result of the hashing is used to find which ring member owns it.

Grafana Fire uses the `fnv32a` hash function, which returns 32-bit unsigned integers so its value can be between `0` and `(2^32)-1`, inclusive.
...
This allows independent determination of what instance of Grafana Fire is the authoritative owner of any specific data.
...
To divide up set of possible tokens (`2^32`) across the available instances within the cluster, all of the running instances of a given Grafana Fire component, such as the ingesters, join a hash ring.
The hash ring is a data structure that splits the space of the tokens into multiple ranges, and assigns each range to a given Grafana Fire ring member.
...
To replicate the data across multiple instances, Grafana Fire finds the replicas by starting from the authoritative owner of the data and walking the ring clockwise.
...
Grafana Fire receives an incoming sample for the series `{__name__="cpu_seconds_total",instance="1.1.1.1"}`.
It hashes the series’ labels, and the result of the hashing function is the token `3`.

To find which ingester owns token `3`, Grafana Fire looks up the token `3` in the ring and finds the ingester that is registered with the smallest token larger than `3`.
...
By default, Grafana Fire replicates each series to three ingesters.
After finding the authoritative owner of the series, Grafana Fire continues to walk the ring clockwise to find the remaining two instances where the series should be replicated.
...
There are several Grafana Fire components that need a hash ring.
...
## How the hash ring is shared between Grafana Fire instances

Hash ring data structures need to be shared between Grafana Fire instances.
To propagate changes to the hash ring, Grafana Fire uses a key-value store.
...
Grafana Fire primarily uses the hash ring for sharding and replication.
Features that are built using the hash ring:
...
- **Zone-aware replication**: Zone-aware replication is the replication of data across failure domains and can be optionally enabled in Grafana Fire. For more information, see [configuring zone-aware replication]({{< relref "../../configure/configuring-zone-aware-replication.md" >}}).
- **Shuffle sharding**: Grafana Fire optionally supports shuffle sharding in a multi-tenant cluster, to reduce the blast radius of an outage and better isolate tenants. For more information, refer to [configure shuffle sharding]({{< relref "../../configure/configuring-shuffle-sharding/index.md" >}}).
