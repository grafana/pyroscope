...
public class OrderService {
    ...
    public synchronized void findNearestVehicle(int searchRadius, String vehicle) {
        Pyroscope.LabelsWrapper.run(new LabelsSet("vehicle", vehicle), () -> {
            ...
            waitForVehicleResponse(searchRadius);
    ...
    private void waitForVehicleResponse(int searchRadius) {
        try {
            Thread.sleep(OP_DURATION.multipliedBy(searchRadius).toMillis());
    ...
    private void checkDriverAvailability(int searchRadius) {
        ...
        waitForDriverConfirmation(searchRadius);
    ...
    private void waitForDriverConfirmation(int searchRadius) {
        try {
            Thread.sleep(OP_DURATION.multipliedBy(searchRadius).toMillis());
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    private void mutexLock(int searchRadius) {
        ...
        waitForMutexRelease(searchRadius);
    }

    private void waitForMutexRelease(int searchRadius) {
        try {
            Thread.sleep(OP_DURATION.multipliedBy(30L * searchRadius).toMillis());
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        }
    }
